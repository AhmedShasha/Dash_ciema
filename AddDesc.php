...
$counter = $core->getDesc($count);

$decs = array();
$decszz = $core->getDesc($shift);
...
if ($_POST) {
    ...
    if (/*$resp->isValid() == */false) {
        ...
        //Captcha is valid
        $add = $core->addDesc($_POST);
        $_POST["control"] = $_SESSION["control"]["id"];

        if (!$add) {
            $_SESSION["error"] = $core->getError();
        } else {
            $_SESSION["success"] = "New Twsef successfully created";
...
<body>
    <div class="DashboardContainer">
        ...
        <div class="card rounded mb-4">
            <h4 class="card-title bg-info text-white rounded p-2">Add Twsef</h4>

            <div class="card-body rounded">
                <form action="" method="POST" enctype="multipart/form-data" class="row">

                    <div class="form-groub col-md-4 col-sm-4">
                        <label for="التوصيف">Type Of Twsef :</label>
                        <input type="text" class="form-control" id="التوصيف" name="التوصيف" placeholder="نوع التوصيف" 
                            value="<?php if (array_key_exists("التوصيف", $_POST)) print $_POST["التوصيف"]; ?>" required>
                    </div>
                    ...
                    <tbody class="thead-light">
                        <tr>
                            ...
                            <td style="width: 185px;">
                                ...
                                <a href="UpdateDesc.php?id=<?= $value['id'] ?>" class="btn btn-primary btn-sm"><i class="fa fa-edit mr-1"></i>Edit</a>

                                <form action="DeleteDesc.php?id=<?= $value['id'] ?>" method="POST" style="display:inline-block;">
